...
RUN echo "01 * * * * /app/env/bin/python /app/manage.py runcrons >> /tmp/cron.log 2>&1" >> /etc/crontabs/app
RUN echo "app ALL=(root) NOPASSWD:SETENV: /usr/sbin/crond" >> /etc/sudoers 
